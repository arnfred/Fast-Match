...
def match(Metric_Cache query_cache, numpy.ndarray[numpy.uint8_t, ndim=3] target_img, object options = {}) :
    ...
    def get_matches(double tau, object thumb_tau = None) :
        thumb_tau = tau if thumb_tau == None else thumb_tau
        positions_iter = itertools.chain(thumb_positions[thumb_ratios<thumb_tau])
        matches = do_iter(positions_iter, query_cache, target_cache, tau = tau, radius = radius, log = log)
        return matches
    return get_matches


cdef do_iter(object positions, Metric_Cache cache, Grid_Cache target_grid, double tau = 0.85, int radius = 100, object log = None) :
    # Declare datatypes
    ...
    while True :
        try :
            ...
            if not has_matched.get((col, row, query_col, query_row), False) :
                ...
                result_pos, ratios = match_position((query_pos, target_pos), cache, target_grid, radius = radius)
